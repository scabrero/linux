 ...
 * searchs all matching tcons to deliver it and then unregisters/registers
...
static int cifs_swn_handle_notification(const struct cifs_swn_notification *not)
{
	 ...
	 * Account how many tcons match to drop their references in case
	 * of having to unregister and register for a new address.
	 */
	spin_lock(&cifs_tcp_ses_lock);
	list_for_each_entry(server, &cifs_tcp_ses_list, tcp_ses_list) {
		if (SERVER_IS_CHAN(server)) {
